...
echo "<div class='container'><div class='col-md-3'></div><div class='col-md-6'>
    ...
    if (isset($_POST['email'])){
      $sql = "SELECT * FROM `hermes`.`users` WHERE email = '".$email."'";
      $query = $db->query($sql);
      ...
      $sql = "INSERT INTO `hermes`.`users` (`display_name`, `email`, `user_group`, `password`) VALUES ('".$name."', '".$email."', 'User', '".$password_crypt."')";
      $db->exec($sql);
      $sql = "SELECT * FROM `hermes`.`users` WHERE email = '".$email."'";
      $query = $db->query($sql);
      $row_count = $query->rowCount();
